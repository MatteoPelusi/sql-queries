...
SELECT albums.id, albums.title, AVG(tracks.milliseconds) AS average_length
FROM albums
...
SELECT albums.id, albums.title, SUM(tracks.unit_price) AS total_price
FROM albums
JOIN tracks ON (tracks.album_id = albums.id)
GROUP BY albums.id
...
SELECT tracks.id, tracks.name, SUM(invoice_lines.quantity) AS total_sold
FROM tracks
